"""Display files with Linux-style cat options."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(prog="xlade cat", description="Display one or more xLaDe files.")
...
def render_file(path: Path, args: argparse.Namespace) -> str:
    """Render one file according to the selected options."""
...
def run(arguments: list[str]) -> int:
    ...
    for filename in args.files:
        ...
        if not path.is_file():
            ...
            continue
        try:
            rendered_files.append(render_file(path, args))
